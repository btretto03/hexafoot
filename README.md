...
- `TelaBase` (classe abstrata) é estendida por todas as oito telas do jogo (`MainMenuView`, `ConvocacaoView`, `EscalacaoTaticaView`, `HubView`, `SimulacaoPartidaView`, `TabelasChaveamentoView`, `CarregarJogoView`, `FeaturePlaceholderView`). O `GameNavigator` troca de tela manipulando sempre a referência polimórfica `ScreenView`, sem precisar saber qual tela concreta está recebendo.
...
A documentação Javadoc, este README, o manual do jogador e a refatoração que introduziu `TelaBase` como classe abstrata foram feitos com auxílio de inteligência artificial, conforme previamente autorizado para as atividades. O conteúdo foi conferido em relação ao código e aos comandos do projeto.
